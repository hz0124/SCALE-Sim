...
class Sensenova_sim():
    def __init__(self, hardware_type="ours", task="GenEval"):
        """
        SenseNova model (rebuttal need-2). Independent copy of Janus_sim with
        SenseNova structure constants; supports only MM (MM-Vet, text-only) and
        GenEval (text->image). GQA like Bagel (num_head_q != num_head_kv).
        Args:
        ...
        self.result_path = os.path.join(PROJECT_ROOT, "results", "sensenova")
        # --- SenseNova structure (given) ---
        self.num_layer = 42
        self.text_input_len = 1
        # GenEval diffusion activation token length = 4096 (given). Split into a
        # small text cross-attn window (text_attn) + image/VAE tokens (vae_attn),
        # image_input_len = text_attn + vae_attn (Janus convention).
        # TODO(data §B): confirm the text_attn / vae_attn split for SenseNova
        # GenEval. Placeholder mirrors Janus (text_attn=2); MM ignores these.
        self.image_input_len = 4096
        self.text_attn = 2          # TODO(§B) placeholder
        self.vae_attn = 4094        # TODO(§B) placeholder (= image_input_len - text_attn)
        self.num_head_kv = 8        # GQA
        self.num_head_q = 32
        self.dim = 4096
        self.head_dim = 128
        self.upshape = 12288        # intermediate_size (gated SwiGLU -> ffn_up x2)
        self.tile = 64
        ...
        self.t2_hard = True
        self.t3_soft = True
        # T3 image-FFN INT4 fraction (rest is FP16). Dual-array static split:
        # f on the FP-INT4 array, (1-f) on FP-FP. Default 0.5 (= width-64 merge).
        # Sweepable for the precision-vs-perf study (REBUTTAL §B-hw).
        self.ffn_int4_frac = 0.5
    ...
    def read_from_json(self, cfg_path=None):
        if cfg_path is None:
            ...
            cfg_path = os.path.join(PROJECT_ROOT, "topologies", "sensenova", "config.json")
        ...
        # Per-task activation length (GenEval=4096, GenEdit=4484): SenseNova's
        # generated-image token count differs by task, so allow JSON override
        # (Janus/Bagel keep it fixed in __init__). text_attn/vae_attn similarly.
        self.image_input_len = cfg.get("image_input_len", self.image_input_len)
        self.text_attn = cfg.get("text_attn", self.text_attn)
        self.vae_attn = cfg.get("vae_attn", self.vae_attn)
        ...
        self.ffn_int4_frac = cfg.get("ffn_int4_frac", self.ffn_int4_frac)
        if self.hardware_type == 'ours' and not self.t3_soft:
            self.array_width_ffn = self.array_width_fp
        elif self.hardware_type == 'ours':
            # T3 image FFN: INT4 fraction f runs on the FP-INT4 array, (1-f) on
            # FP-FP, both array_width_half wide, in parallel -> makespan = larger
            # half. Effective merged width = array_width_half / max(f, 1-f):
            # f=0.5 -> 64 (full merge); lower f -> FP16 half dominates -> narrower
            # effective width -> slower FFN (the precision/perf trade being swept).
            f = self.ffn_int4_frac
            self.array_width_ffn = self.array_width_half / max(f, 1.0 - f)
        else:
    ...
    def build_topologies(self, kv_len=0, is_gen_text=True, part='all'):
        """
        生成SenseNova模型单层拓扑结构，并保存到相应的csv文件中（结构基于 Janus）
        ...
        """
        # 创建输出目录
        output_dir = os.path.join(PROJECT_ROOT, "topologies", "sensenova")
    ...
    # devlog_rebuttal/docs/NEED9_DISAGG.md §6. SenseNova is GQA (num_head_q=32, num_head_kv=8),
    # like Bagel: text attention multiplies by num_head_q and streams KV per query head
    # (same GQA KV-reuse simplification as Bagel_sim). No disagg hw type for SenseNova.
        ...
        if self.hardware_type == 'ours':
            ...
            if self.t3_soft:
                img_ffn_kwargs = {"precision": "fp16", "weight_precision": "int4",
                                  "mac_split": {"fp16": 1.0 - self.ffn_int4_frac,
                                                "w4a16": self.ffn_int4_frac}}
            else:
    ...
    def run_model(self):
        ...
        # 记录仿真开始
        self._append_to_log("======= SenseNova Model Simulation Started =======")
        ...
        self._append_to_log("======= SenseNova Model Simulation Completed =======")
